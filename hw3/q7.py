...
def get_best_inlier_group_indices(ref_points, seq_points):
    ...
    iteration = 1
    largest_group = list()
    while len(largest_group) < 3:
        inlier_groups = get_inlier_groups_indices(ref_points, seq_points, 100, max_dist=1*iteration)
        iteration += 1
        largest_group = max(inlier_groups, key=len)
    return largest_group


def get_inlier_groups_indices(ref_points, seq_points, repeats, max_dist=5):
    if len(ref_points) != len(seq_points):
        raise Exception('Bad lengths')
    groups = list()
    for i in range(repeats):
        ...
        for j in range(len(ref_points)):
            estimated_source_point = utils.transform(seq_points[j], a, b)
            dist = spatial.distance.euclidean(estimated_source_point, ref_points[j])
            if dist < max_dist:
                group.append(j)
